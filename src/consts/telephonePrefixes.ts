...
const sortPhonePrefixes = () => {
  const { countries } = numberPrefixes;
  const sortedPrefixes = countries
    .map(({ code }) => +code.replaceAll(' ', ''))
    .sort((a, b) => a - b)
    .map((element) => `+${element}`);
  return countries.map((element, index) => ({ code: sortedPrefixes[index], name: element.name }));
};

numberPrefixes.countries = sortPhonePrefixes();

export const TelephonePrefixes = numberPrefixes.countries.map((item) => item.code);
