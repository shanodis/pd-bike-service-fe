...
const InformationsForm: FC<InformationsFormProps> = ({ readonly }) => {
  ...
  const [customerBikes] = useFetchData<SelectOptionModel[]>(
    '/dictionaries/bikes',
    { dataMapper: customerBikeMapper, params: customerBikeParams },
  );
  ...
  const radioFontChange = (currentFormChangeState: string) =>
    formChange === currentFormChangeState ? 'fw-bold' : 'fw-normal bg-white text-secondary border-secondary-light';
  ...
  return (
    <Form>
      ...
      <Row className='text-end'>
        <Col className='mt-5'>
          {!readonly && (
            <SubmitButton variant='success-lighter' type='submit' className='ms-3 text-white fw-bold'>
